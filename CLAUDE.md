...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - En **modo posición**, `#split-zoom` toma la clase `.folio-box` y se dimensiona con las **proporciones DIN de la hoja** (`fitRect` con el folio orientado al contenido, centrado en el viewport); la imagen de la capa se escala relativa al folio (`layerFmt`/`jobFmt`, serie A → misma proporción) y se arrastra dentro (`startLayerDrag`, dividiendo el delta por el zoom). Un `.folio-outline` (recuadro discontinuo, `pointer-events:none`) se dibuja POR ENCIMA de la imagen para ver si el contenido se sale de la hoja al arrastrar. En **modo split**, `#split-zoom` se ajusta a la imagen fuente (`fitRect` del aspecto natural) y muestra las líneas de corte.
