...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - **Alta resolución**: el preview del editor se carga con `?hires=1` (render a ~2000 px una sola vez al abrir/rotar vía `generate_rotated_preview(width_px=2000)`), así el zoom (CSS puro) se mantiene nítido sin recalcular en cada paso.
