...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - En **modo posición**, `#split-zoom` toma la clase `.folio-box` con el folio **en vertical (base)**, igual que la composición real de la hoja (`folio = orientMM(jobFmt, false)`), para que los offsets cuadren con la aurrebista; la imagen de la capa se escala relativa al folio según SU orientación (`layer = orientMM(layerFmt, landscape)`) y se arrastra dentro (`startLayerDrag`, dividiendo el delta por el zoom). Un `.folio-outline` (recuadro discontinuo, `pointer-events:none`) se dibuja POR ENCIMA para ver si el contenido se sale de la hoja, y un `.folio-ghost` (las demás capas, atenuadas) POR DEBAJO para no solaparlas (`GET /api/sheets/{id}/ghost?exclude=printId`). En **modo split**, `#split-zoom` se ajusta a la imagen fuente y muestra las líneas de corte.
