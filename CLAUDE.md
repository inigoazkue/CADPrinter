...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - En **modo posición**, `.split-preview-wrap` recibe la clase `.folio-box` y se dimensiona (vía JS, `applyPositionTransform`) con las **proporciones DIN de la hoja** (orientadas según el contenido); la imagen de la capa se escala relativa al folio (`layerFmt`/`jobFmt`, serie A → misma proporción) y se arrastra dentro. Así se ve si la capa cabe en la hoja. En **modo split** se quita `.folio-box` y el wrap vuelve a ajustarse a la imagen.
...
- `GET /api/prints/{id}/preview?rotation=N` sirve un PNG rotado en vivo (sin tocar el fichero) vía `generate_rotated_preview()`. `?placed=1` renderiza la capa **colocada en su folio** (lienzo orientado al contenido, en su offset) reutilizando `generate_sheet_preview([path], fmt, ...)` → la miniatura de cada capa muestra rotación + posición tal como queda en la hoja (la miniatura del job usa `?placed=1`; la de "Iturriak" usa el preview crudo). Sin parámetros sirve el preview cacheado con `Cache-Control: no-cache` (revalida por `Last-Modified`). El fichero placed es `previews/placed_{id}.png` y se borra al eliminar el print.
...
- **Escala y orientación (folio)**: tanto la aurrebista (`generate_sheet_preview`) como el export colocan cada capa a su **tamaño nativo** (sin estirar para llenar la hoja) en su offset, y el lienzo (folio) toma la **orientación del contenido** vía `_oriented_canvas()` (si la capa es apaisada, la hoja es apaisada). Así, al rotar una capa (botón ↻), el folio entero gira y la escala se mantiene sin deformar.
- **Caché de estáticos**: el frontend se sirve con `NoCacheStaticFiles` (subclase de `StaticFiles`) que añade `Cache-Control: no-cache` a todo (revalida por ETag/Last-Modified → 304 si no cambia). Además, `index.html` carga `app.js` con un token dinámico (`/js/app.js?t=Date.now()` vía un `<script>` que crea el tag) para que ni el navegador ni un proxy corporativo puedan servir un `app.js` rancio: una vez que llega este `index.html`, el JS se auto-refresca siempre.
