...
jobs (id, name, format A0–A6, is_current, source_user, created_at)
  └── sheets (id, job_id, name, order_num, rotation)
        └── prints (id, sheet_id, job_id, filename, original_name,
                    ...
                    offset_x_mm, offset_y_mm, scale, tile_col, tile_row)

user_active_jobs (source_user PK, job_id FK → jobs)
```

- **Orden de las capas dentro de una hoja**: por **orden de llegada** (`ORDER BY received_at, id`) en todas las consultas (grid, aurrebista, ghost). NO por `order_num` (que provocaba un orden raro al mezclar tiles con páginas). La capa que llega después se apila encima.
- **Escala real de capa** (`prints.scale`, REAL DEFAULT 1.0): factor de tamaño manual de cada capa (1.0 = tamaño nativo 1:1), anclado en la esquina superior izquierda del offset. Se aplica en `_compose_sheet` (`ox+sw*sc, oy+sh*sc`) → afecta a aurrebista, miniatura `?placed=1` y export. Lo controlan los botones `−/+/▢` del editor (ver más abajo). `PrintEdit` lo guarda; el botón ↻ de la miniatura lo preserva.
...
- **Conteos del sidebar** (`countLabel`): estilo euskera — singular con el número detrás (`orri 1`, `geruza 1`) y plural con el número delante (`2 orri`, `3 geruza`). `db_list_jobs` **excluye la hoja "Iturriak"** (y su print original) de `sheet_count`/`print_count`, porque no es una hoja de salida real.
...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - **Zoom de visualización (rueda) vs. escala real (botones)**: el preview vive en un contenedor `#split-zoom` (dentro del viewport `.split-preview-wrap`, tamaño fijo `56vh`/`overflow:hidden`) al que se le aplica `transform: translate(pan) scale(zoom)`. **La rueda del ratón** hace zoom de **solo visualización** (`zoomAt`, 1..8, hacia el cursor) — no afecta a la salida. **Los botones `−`/`+`/`▢`** (`zoom-out`/`zoom-in`/`zoom-reset`) cambian la **ESCALA REAL** de la capa en **modo posición** (`scaleStep` → `splitState.layerScale`, pasos ×1.05, límites 0.1..10) → se ve más grande/pequeña en la salida final; `▢` (`resetScaleAndView`) pone la escala real al 100% y recentra la vista. En **modo split** los botones siguen siendo zoom de visualización (no hay una sola capa que escalar). El `#zoom-scale-label` muestra el % de escala real (oculto en modo split). Al guardar (modo posición) se envía `scale` en `POST /api/prints/{id}/edit`. La imagen y las líneas de corte van dentro de `#split-zoom`, así el zoom de vista las escala juntas; el arrastre de líneas (`makeDraggable`) calcula la fracción con `getBoundingClientRect()` del contenedor (a prueba de zoom/pan). El tamaño base de la capa sin escalar se guarda en `splitState._layerBaseW/_layerBaseH` para reescalar en vivo (`applyLayerScale`).
  - En **modo posición**, `#split-zoom` toma la clase `.folio-box` con el folio **en vertical (base)**, igual que la composición real de la hoja (`folio = orientMM(jobFmt, false)`), para que los offsets cuadren con la aurrebista; la imagen de la capa se escala relativa al folio según SU orientación (`layer = orientMM(layerFmt, landscape)`) y se arrastra dentro (`startLayerDrag`, dividiendo el delta por el zoom). Un `.folio-outline` (recuadro discontinuo, `pointer-events:none`) se dibuja POR ENCIMA para ver si el contenido se sale de la hoja. **Esta línea discontinua ES el borde del papel**: `.folio-box` ya no lleva borde sólido (se quitó) y el outline se dibuja con `outline: dashed` + `outline-offset:-1.5px` para quedar exactamente en el borde del folio (no metido hacia dentro), de modo que datos que caigan sobre la línea sí se imprimen. Además hay un `.folio-ghost` (las demás capas, atenuadas) POR DEBAJO para no solaparlas (`GET /api/sheets/{id}/ghost?exclude=printId`). En **modo split**, `#split-zoom` se ajusta a la imagen fuente y muestra las líneas de corte.
  - **Alta resolución + blanco transparente**: el preview del editor se carga con `?hires=1` → `generate_editor_preview(width_px=2000)`, que renderiza a ~2000 px (una sola vez al abrir/rotar; el zoom es CSS puro) y trata el **blanco como transparente** (keying con numpy; fallback a fondo transparente si no hay numpy) para que el fantasma de las otras capas se vea a través de las zonas blancas. `.split-zoom` tiene fondo blanco (papel) para las capas keyed.
...
- `POST /api/prints/{id}/edit` (modelo `PrintEdit`: `rotation`, `offset_x_mm`, `offset_y_mm`, `scale`): si `rotation≠0` hornea la rotación en el PDF con `pdf_utils.apply_rotation_to_pdf()` (reescribe el fichero rotado vía `show_pdf_page(rotate=)` a temp + `os.replace`), regenera el preview y re-detecta el formato. Guarda los offsets y la **escala real** (`scale>0`, si no 1.0). Broadcast `job_updated`.
- `POST /api/prints/{id}/reuse` (modelo `PrintReuse`: `sheet_id` opcional): **copia** un print (típicamente una Iturria) como nueva capa normal habilitada en una hoja (por defecto, la primera hoja que NO sea "Iturriak"), reseteada a offset 0 / escala 1 / `tile_col` NULL. **Duplica el fichero PDF** (`shutil.copyfile`, id nuevo) para que borrar una copia no afecte a la otra. Broadcast `job_updated`. Lo usa el botón "Txertatu" del desplegable de Iturriak.
...
- `POST /api/prints/{id}/split` divide un print grande en piezas. Parámetros: `cols`, `rows`, `tile_format`, `overlap_mm`, `col_positions` (norm. 0..1), `row_positions`, `rotation`. **Las piezas NO se ponen lado a lado: se SUPERPONEN.** Se crean todas en la hoja original como capas normales habilitadas (`tile_col`/`tile_row` se guardan como metadato, `offset_x_mm/y=0` al inicio → empiezan apiladas). El original pasa a una hoja "Iturriak" (disabled). La aurrebista de esa hoja se genera con `pdf_utils.generate_sheet_preview()` en **formato tile** (p.ej. A3), superponiendo las piezas con sus offsets. El usuario arrastra cada pieza (modo posición del editor) para componer un único A3. **Tras un split, el formato del trabajo se actualiza automáticamente al formato del tile MÁS GRANDE producido en ese trabajo** (menor número A, p.ej. A2 > A3): se consulta `SELECT DISTINCT format FROM prints WHERE job_id=? AND tile_col IS NOT NULL` y se toma el `min` por `int(f[1:])`. Así el formato deja de quedarse en el del original (que ahora está en Iturriak) y, si después se hacen tiles más pequeños, se conserva el mayor. **Solo los splits cambian el formato automáticamente**; añadir páginas (tile_col NULL) nunca lo altera.
- **Iturriak (desplegable, no es una hoja)**: las hojas "Iturriak" YA NO se muestran en el listado de hojas. `renderJobDetail` las salta (`if (sheet.name === 'Iturriak') continue`) y reúne sus prints en un **desplegable** en la cabecera del trabajo, junto a Formatua/Aktibatu (`#iturriak-dd` → botón `#btn-iturriak` con badge de conteo `#iturriak-count` + panel `#iturriak-panel`). `renderIturriakDropdown(job)` puebla el panel (oculto si no hay iturriak): cada iturria muestra su aurrebista (`?placed`-no, preview crudo) + nombre + botón **"⬇ Txertatu 1. orrian"** que llama a `API.reusePrint(p.id)` (sin `sheet_id` → primera hoja real) para re-insertarla como capa y volver a usarla. El panel se abre/cierra con el botón y se cierra al hacer clic fuera.
- `GET /api/prints/{id}/preview?rotation=N` sirve un PNG rotado en vivo (sin tocar el fichero) vía `generate_rotated_preview()`. `?hires=1` renderiza en alta resolución (~2000 px) para el editor (zoom nítido). `?placed=1` renderiza la capa **colocada en su folio** (lienzo orientado al contenido, en su offset) reutilizando `generate_sheet_preview([path], fmt, ...)` → la miniatura de cada capa muestra rotación + posición tal como queda en la hoja (la miniatura del job usa `?placed=1&t=Date.now()` —el token evita caché de navegador/proxy para que el giro/movimiento se refleje al instante—; la de "Iturriak" usa el preview crudo). Sin parámetros sirve el preview cacheado con `Cache-Control: no-cache` (revalida por `Last-Modified`). El fichero placed es `previews/placed_{id}.png` y se borra al eliminar el print.
...
- **Escala (folio)**: tanto la aurrebista (`generate_sheet_preview`) como el export colocan cada capa a su **tamaño nativo × `scale`** (sin estirar, escala real manual; 1.0 = nativo) en su offset vía `_compose_sheet()`. Todos los dicts de offset (`{x_mm, y_mm, scale}`) que se pasan a la composición incluyen `scale`.
