...
- **Editor unificado de geruza (capa)**: al hacer clic en la miniatura de cualquier print se abre el modal `#modal-split`, que es un editor con DOS modos según `cols`/`rows`:
  ...
  - En **modo posición**, `.split-preview-wrap` recibe la clase `.folio-box` con **proporciones DIN de la hoja** vía CSS `aspect-ratio: var(--folio-ar)` (la JS `applyPositionTransform` calcula `--folio-ar = folioW/folioH` orientado al contenido y altura fija `min(52vh,400px)`); la imagen de la capa se escala relativa al folio (`layerFmt`/`jobFmt`, serie A → misma proporción) y se arrastra dentro. Así se ve si la capa cabe en la hoja. En **modo split** se quita `.folio-box` y el wrap vuelve a ajustarse a la imagen.
