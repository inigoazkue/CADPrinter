...
- **Escala y orientación (folio)**: tanto la aurrebista (`generate_sheet_preview`) como el export colocan cada capa a su **tamaño nativo** (sin estirar) en su offset vía `_compose_sheet()`. El lienzo se orienta a la capa de **mayor área** (`_oriented_canvas()`), no a la primera, para que rotar una capa pequeña NO voltee la hoja entera.
- **Rotación de hoja (manual)**: cada `sheet` tiene columna `rotation` (0/90/180/270, con migración). La aurrebista tiene su propio botón ↻ (`renderSheet` → `PATCH /api/sheets/{id}` con `rotation`, modelo `SheetUpdate`) que rota la **salida final** como unidad (contenido+papel). Es independiente de la rotación de cada capa: rotar una capa solo gira su contenido (su miniatura `?placed=1` se auto-orienta), mientras que la aurrebista solo gira si el usuario pulsa su botón. `_compose_sheet(rotation=N)` aplica la rotación al componer; el export usa `sheet_rotations`.
