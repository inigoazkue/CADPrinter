...
- **Caché de estáticos**: el frontend se sirve con `NoCacheStaticFiles` (subclase de `StaticFiles`) que añade `Cache-Control: no-cache` a todo, para que `app.js`/`style.css`/`index.html` no queden cacheados rancios tras un deploy (revalidan por ETag/Last-Modified → 304 si no cambian).
